...
"""
Embedding service — CPU all-MiniLM-L6-v2 for the RAG pipeline.

Embeds incoming chat queries to 384-dim vectors for Qdrant cosine search. The
indexer (scripts/index_with_embeddings.py) uses the same model, so index vectors
match query vectors. Runs CPU-only on the T5810 (vLLM owns the GPUs). Mirrors the
reranker service pattern (port 8006); this is port 8005.
"""
